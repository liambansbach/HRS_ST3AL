...
class SequenceModel(Node):
    ...
    def cb(self, msg: CubeBBoxList) -> None:
        ...
        - build buffer out of msg
        - derive motion
        - FSM to determine picks and places
        """
        ...
        for cube in self.cube_ids:
            ...
            if found_bbox is not None:
                ...
                obs = {
                    ...
                    "h": h,
                    "angle": angle,
                    "x": px,   # keep x/y for minimal downstream changes
                    "y": py,
                    "z": 0.0,
                }
    ...
    def derive_motion(self) -> None:
        """
        Compute moving/still/occluded per cube using a true window-based speed,
        modeled after the old stable node.

        IMPORTANT: this assumes obs["t"] stored in histories is rclpy.time.Time
        (i.e., from self.get_clock().now()).
        """
        for cube in self.cube_ids:
